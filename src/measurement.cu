...
namespace QuaSARQ {
    ...
    __global__ void measure_determinate_init(Table* inv_xs, Table* inv_zs, Signs* inv_ss, DeviceLocker* dlocker, 
        ...
        for_parallel_x(i, num_gates) {
            ...
            if (m.pivot == MAX_QUBITS) {
                ...
                word_std_t qubit_word = 0;
                // Check if destabilizers commute.
                // TODO this can be done in parallel as well using atomicMin.
                for (grid_t j = 0; j < num_qubits; j++) {
                    qubit_word = (*inv_xs)[j * num_words_minor + q_w];
                    if (qubit_word & q_mask) {        
                        m.measurement = inv_ss->unpacked_data()[j + num_qubits];
                        break;
                    }
                }
            }
    ...
    INLINE_DEVICE void row_to_aux(DeviceLocker& dlocker, int& measurement, word_std_t* aux, const Table& inv_xs, const Table& inv_zs, const int* inv_ss, const grid_t& generator_idx, const size_t& num_words_minor) {
        const grid_t tx = threadIdx.x, BX = blockDim.x;
        const grid_t shared_tid = threadIdx.y * BX * 2 + tx;
        const grid_t w = blockIdx.x * BX + tx;
        if (!w) {
            measurement = inv_ss[generator_idx];
            //int old_value = atomicAdd(&measurement, inv_ss[generator_idx]);
            //assert(old_value == UNMEASURED);
        }
        word_std_t* aux_xs = aux;
        word_std_t* aux_zs = aux + blockDim.x;
        if (w < num_words_minor && tx < num_words_minor) {
            const grid_t qubits_word_idx = generator_idx * num_words_minor + w;
    ...
    INLINE_DEVICE void row_aux_mul(DeviceLocker& dlocker, Gate& m, word_std_t* aux, int* aux_power, Table& inv_xs, const Table& inv_zs, const int* inv_ss, const grid_t& src_idx, const grid_t& aux_idx, const size_t& num_words_minor) {
        word_std_t* aux_xs = aux;
        word_std_t* aux_zs = aux + blockDim.x;
        ...
        if (!w) {
            CHECK_SIGN_OVERFLOW(src_idx, m.measurement, inv_ss[src_idx]);
            m.measurement += inv_ss[src_idx];
        }
        if (w < num_words_minor && tx < num_words_minor) {
        ...
        // if (!w) {
        //     int old_value = atomicAdd(&m.measurement, inv_ss[src_idx]);
        //     CHECK_SIGN_OVERFLOW(src_idx, old_value, inv_ss[src_idx]);
        // }
    }

    // Measure a determinate qubit.
    INLINE_DEVICE void measure_determinate_qubit(DeviceLocker& dlocker, Gate& m, Table& inv_xs, Table& inv_zs, int* inv_ss, word_std_t* aux, int* aux_power, const size_t num_qubits, const size_t num_words_minor) {
        const qubit_t q = m.wires[0], q_w = WORD_OFFSET(q);
        const word_std_t q_mask = BITMASK_GLOBAL(q);
        word_std_t qubit_word = 0;
        // Check if destabilizers commute.
        for (grid_t j = 0; j < num_qubits; j++) {
            qubit_word = inv_xs[j * num_words_minor + q_w];
            if (qubit_word & q_mask) {        
                row_to_aux(dlocker, m.measurement, aux, inv_xs, inv_zs, inv_ss, j + num_qubits, num_words_minor);
                for (grid_t k = j + 1; k < num_qubits; k++) {
                    qubit_word = inv_xs[k * num_words_minor + q_w];
                    if (qubit_word & q_mask) {
                        row_aux_mul(dlocker, m, aux, aux_power, inv_xs, inv_zs, inv_ss, k + num_qubits, j + num_qubits, num_words_minor);
                    }
                }
                break;
            }
    ...
    __global__ void measure_determinate(Table* inv_xs, Table* inv_zs, Signs* inv_ss, DeviceLocker* dlocker, 
        ...
        for_parallel_y(i, num_gates) {
            ...
            if (m.pivot == MAX_QUBITS) {
                assert(m.size == 1);
                measure_determinate_qubit(*dlocker, m, *inv_xs, *inv_zs, inv_ss->unpacked_data(), aux, aux_power, num_qubits, num_words_minor);
            }
        }
    }
    ...
    void Simulator::measure(const size_t& p, const depth_t& depth_level, const cudaStream_t* streams, const bool& reversed) {
        ...
        is_indeterminate_outcome<<<bestGridMeasure, bestBlockMeasure, 0, kernel_stream>>>(gpu_circuit.gates(), gpu_circuit.references(), inv_tableau.xtable(), num_qubits, num_gates_per_window, num_words_minor);
        if (options.sync) {
            LASTERR("failed to launch is-indeterminate-outcome kernel");
            SYNC(kernel_stream);
        }
        ...
        measure_determinate<<<nBlocks_det, nThreads_det, smem_size_det, kernel_stream>>>(XZ_TABLE(inv_tableau), inv_tableau.signs(), locker.deviceLocker(), gpu_circuit.gates(), gpu_circuit.references(), num_gates_per_window, num_qubits, num_words_minor);
        if (options.sync) {
            LASTERR("failed to launch measure-determinate kernel");
            SYNC(kernel_stream);
        }
