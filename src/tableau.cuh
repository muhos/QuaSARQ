    ...
    */
    ...
    public:
        ...
        size_t alloc(const size_t& num_qubits, const size_t& max_window_bytes, const size_t& forced_num_partitions = 0) {
            ...
            num_partitions = 1;
            while ((forced_num_partitions && forced_num_partitions > num_partitions) || (expected_capacity_required >= cap_before && _num_words_major > 1)) {
            ...
            assert(num_partitions == 1 && _num_words_major == get_num_words(num_qubits)
                || num_partitions > 1 && num_partitions * _num_words_major <= _num_qubits_padded);
    ...
    public:
        ...
        size_t alloc(const size_t& num_qubits, const size_t& max_window_bytes, const size_t& forced_num_partitions = 0) {
            ...
            // bind the allocated GPU pointers to the host object,
            // then transfer it to the GPU.
            h_xs->alloc(_num_words, _num_words_major, _xs_data);
            h_zs->alloc(_num_words, _num_words_major, _zs_data);
            h_ss->alloc(num_sign_words, _ss_data);
