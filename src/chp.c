...

{
	...
	{
		if (q->x[p + q->n][b5] & pw)
			ran = 1; // if a Zbar does NOT commute with Z_b (the
		if (ran)
			break; // operator being measured), then outcome is random
	}
	...
	if (ran)
	{
		...
		q->r[p + q->n] = 2; //2 * (rand() % 2);		// moment of quantum randomness
...

{
	...
	if (argc == (3 + param))
		initstae_(q, h->n, argv[2 + param]);
	else
		initstae_(q, h->n, NULL);
	runprog(h, q);
