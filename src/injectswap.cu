...
namespace QuaSARQ {
        ...
        const   size_t          num_qubits_padded)
    {
        ...
        const size_t stab_idx = TABLEAU_INDEX(q_w, pivot) + TABLEAU_STAB_OFFSET;
        const word_std_t z_stab = (*inv_zs)[stab_idx];
        COMMUTING_FLAG = (z_stab & q_mask) ? 1 : 0;
    }
        ...
        const   size_t          num_qubits_padded)
    {
        for_parallel_x(w, num_words_minor) {
            ...
            collapse_swap_half(xs[c_destab], zs[c_destab], is_commuting, ss[w]);
            collapse_swap_half(xs[c_stab], zs[c_stab], is_commuting, ss[signs_stab_idx]);
        ...
        const   size_t          num_qubits_padded)
	{
        ...
        const size_t q_w = WORD_OFFSET(qubit);
        const word_std_t q_mask = BITMASK_GLOBAL(qubit);
        const size_t word_idx = TABLEAU_INDEX(q_w, pivot) + TABLEAU_STAB_OFFSET;
        return bool(h_zs.words()[word_idx] & q_mask);
	}
	...
	void MeasurementChecker::inject_swap_cpu() {
        ...
        for (size_t w = 0; w < num_words_minor; w++) { 
            ...
            collapse_swap_half(h_xw[c_destab], h_zw[c_destab], commuting, h_ss[w]);
            collapse_swap_half(h_xw[c_stab], h_zw[c_stab], commuting, h_ss[w + num_words_minor]);
        }
    }
