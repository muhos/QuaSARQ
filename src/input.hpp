...
namespace QuaSARQ {
	...
	protected:
		...
		ARG(arg_t a, arg_t x, arg_t t) : arg(a), text(x), type(t), parsed(false) {
			opts().push(this);
		}
	public:
		// global
	...
	public:
		STRING_OPT(arg_t a, arg_t x, arg_t val = nullptr)
		...
		size_t length() const { assert(val != nullptr); return strlen(val); }
