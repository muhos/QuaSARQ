...
constexpr const char* SURFACE_CODE_D10_R3 = "/home/muhos/quantum/github/QuaSARQ/tests/circuits/surface_code_d10_r3.stim";
...
void reset_options() {
    ...
    options.num_shots = 0;
    options.min_measures_write = 100;
    options.streams = 6;
    options.tuner_en = false;
    std::strcpy(options.configpath, "/home/muhos/quantum/github/QuaSARQ/src/kernel.config");
    std::strcpy(options.statepath, "../build/test_simulator.qstate");
    options.check(SURFACE_CODE_D10_R3);
}

void test_surface_code_lifecycle() {
    section("Simulator surface code lifecycle");

    run_test("loads, schedules, streams, and simulates surface code", [] {
        reset_options();
